...
// ---- Dashboard Users (managed by admin) ----
const USERS_KEY = 'macao_dashboard_users';

export const getDashboardUsers   = () => {
  const users = read(USERS_KEY, []);
  if (users.length === 0) {
    const master = { id: 'master-admin', name: 'Jonathan', email: 'jonathan@macaooffroad.com', phone: '+1 809-000-0000', pin: '000000', role: 'admin', active: true, createdAt: new Date().toISOString() };
    write(USERS_KEY, [master]);
    return [master];
  }
  // Ensure all users have an email field (migration for old data)
  let needsWrite = false;
  const migrated = users.map(u => {
    if (!u.email && u.id === 'master-admin') {
      needsWrite = true;
      return { ...u, email: 'jonathan@macaooffroad.com' };
    }
    return u;
  });
  if (needsWrite) write(USERS_KEY, migrated);
  return needsWrite ? migrated : users;
};
export const saveDashboardUsers  = (u) => write(USERS_KEY, u);
export const addDashboardUser    = (user) => {
  const all = getDashboardUsers();
  all.push({ ...user, id: Date.now().toString(), createdAt: new Date().toISOString(), active: true });
  saveDashboardUsers(all);
  return all;
};
export const updateDashboardUser = (id, updates) => {
  let found = false;
  const all = getDashboardUsers().map(u => {
    if (u.id === id) { found = true; return { ...u, ...updates }; }
    return u;
  });
  if (!found) {
    // Fallback: find user by scanning all users (handles ID mismatch from old data)
    const byEmail = getDashboardUsers();
    const target = byEmail.find(u => u.id == id);
    if (target) {
      const fixed = byEmail.map(u => u === target ? { ...u, ...updates } : u);
      saveDashboardUsers(fixed);
      return fixed;
    }
  }
  saveDashboardUsers(all);
...
export const deleteDashboardUser = (id) => {
  const all = getDashboardUsers().filter(u => u.id !== id);
  saveDashboardUsers(all);
  return all;
};
export const findDashboardUser   = (id) => getDashboardUsers().find(u => u.id === id);
export const authenticateDashboardUser = (pin) => {
  return getDashboardUsers().find(u => u.pin === pin && u.active);
};
export const authenticateDashboardUserByEmail = (email) => {
  const users = getDashboardUsers();
  // Try exact email match first
  const user = users.find(u => u.email?.trim().toLowerCase() === email?.trim().toLowerCase() && u.active);
  if (user) return user;
  // Fallback: try partial match (in case of trailing spaces or format issues)
  return users.find(u => u.active && u.email && email && u.email.trim().toLowerCase().includes(email.trim().toLowerCase()));
};
