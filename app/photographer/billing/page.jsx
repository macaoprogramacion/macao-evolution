...
function CierreTurnoPanel({ invoices }) {
  ...
  const turnoInvoices = selectedTurno
    ? todayInvoices.filter(i => i.turno === selectedTurno)
    : [];

  const activeInvoices = turnoInvoices.filter(i => i.status !== 'cancelled');
  const cancelledInvoices = turnoInvoices.filter(i => i.status === 'cancelled');
  const returnedNums = new Set(returns.filter(r => r.status === 'aprobada').map(r => r.invoice));
  const turnoReturns = returns.filter(r => {
    const inv = turnoInvoices.find(i => i.invoiceNumber === r.invoice);
    return inv && r.status === 'aprobada';
  });

  const totalVentas = activeInvoices.reduce((s, i) => s + i.total, 0);
  const totalDevoluciones = turnoReturns.reduce((s, r) => s + r.amount, 0);
  const totalNeto = totalVentas - totalDevoluciones;

  const handlePrint = () => {
    if (!selectedTurno) return;
    const w = window.open('', '', 'width=302,height=800');
    const rows = activeInvoices.map(i =>
      '<tr><td style="padding:3px 0;">' + i.invoiceNumber + '</td><td style="text-align:right;padding:3px 0;">US$ ' + i.total.toFixed(2) + '</td></tr>'
    ).join('');
    const cancelRows = cancelledInvoices.map(i =>
      '<tr><td style="padding:3px 0;text-decoration:line-through;">' + i.invoiceNumber + '</td><td style="text-align:right;padding:3px 0;text-decoration:line-through;">US$ ' + i.total.toFixed(2) + '</td></tr>'
    ).join('');
    w.document.write('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Cierre ' + selectedTurno + '</title>'
      + '<style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:monospace;width:80mm;padding:8px;font-size:12px}'
      + 'h2{text-align:center;font-size:14px;margin-bottom:4px}table{width:100%;border-collapse:collapse}'
      + '.sep{border-top:1px dashed #000;margin:8px 0}.total{font-size:16px;font-weight:bold;text-align:center;margin:8px 0}'
      + '@page{size:80mm auto;margin:0}@media print{body{width:80mm}}</style></head><body>'
      + '<h2>MACAO PHOTOGRAPHY</h2><p style="text-align:center;font-size:11px">CIERRE DE TURNO</p>'
      + '<div class="sep"></div>'
      + '<p><strong>' + selectedTurno + '</strong></p>'
      + '<p>Fecha: ' + todayStr + '</p>'
      + '<div class="sep"></div>'
      + '<p style="font-weight:bold;margin-bottom:4px">VENTAS (' + activeInvoices.length + ')</p>'
      + '<table>' + rows + '</table>'
      + (cancelledInvoices.length > 0 ? '<div class="sep"></div><p style="font-weight:bold;margin-bottom:4px">CANCELADAS (' + cancelledInvoices.length + ')</p><table>' + cancelRows + '</table>' : '')
      + '<div class="sep"></div>'
      + '<table>'
      + '<tr><td>Ventas:</td><td style="text-align:right">US$ ' + totalVentas.toFixed(2) + '</td></tr>'
      + '<tr><td>Devoluciones:</td><td style="text-align:right">-US$ ' + totalDevoluciones.toFixed(2) + '</td></tr>'
      + '</table>'
      + '<div class="sep"></div>'
      + '<div class="total">NETO: US$ ' + totalNeto.toFixed(2) + '</div>'
      + '<div class="sep"></div>'
      + '<p style="text-align:center;font-size:10px">Impreso: ' + new Date().toLocaleString('es-DO') + '</p>'
      + '</body></html>');
    w.document.close();
  ...
  return (
    <div className="flex-1 flex flex-col">
      ...
      <div className="flex gap-4 mb-6 items-end">
        <div className="w-64">
          <label className="block text-white/60 text-xs mb-1.5">Seleccionar Turno</label>
          <select
            value={selectedTurno}
          ...
          >
            ...
            <option value="Turno 9:00">Turno 9:00 AM</option>
            <option value="Turno 12:00">Turno 12:00 PM</option>
            <option value="Turno 3:00">Turno 3:00 PM</option>
          </select>
        </div>
        {selectedTurno && (
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
            onClick={handlePrint}
            className="px-6 py-2.5 bg-[#DC2626] text-white rounded-2xl font-medium flex items-center gap-2">
            <Printer className="w-4 h-4" /> Imprimir Cierre
          </motion.button>
        )}
      </div>
      ...
      {selectedTurno ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
          <div className="bg-black/25 backdrop-blur-xl rounded-3xl p-5 border border-white/20">
            <p className="text-white/60 text-xs mb-1">Facturas Activas</p>
            <p className="text-white text-3xl font-bold">{activeInvoices.length}</p>
            <p className="text-green-400 text-sm mt-1">US$ {totalVentas.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
          </div>
          <div className="bg-black/25 backdrop-blur-xl rounded-3xl p-5 border border-white/20">
            <p className="text-white/60 text-xs mb-1">Cancelaciones</p>
            <p className="text-white text-3xl font-bold">{cancelledInvoices.length}</p>
            <p className="text-red-400 text-sm mt-1">US$ {totalDevoluciones.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
          </div>
          <div className="bg-black/25 backdrop-blur-xl rounded-3xl p-5 border border-[#DC2626]/40">
            <p className="text-white/60 text-xs mb-1">Total Neto</p>
            <p className="text-[#DC2626] text-3xl font-bold">US$ {totalNeto.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
          </div>
        </div>
      ) : (
        <div className="flex-1 flex items-center justify-center text-white/40">
          <p>Selecciona un turno para ver el resumen</p>
        </div>
      )}

      {selectedTurno && (
        <div className="flex-1 bg-black/25 backdrop-blur-xl rounded-3xl p-5 border border-white/20 overflow-auto">
          <h3 className="text-white font-semibold mb-4">Detalle de Facturas</h3>
          <table className="w-full">
            <thead>
              <tr className="text-white/70 text-xs border-b border-white/10">
                <th className="text-left py-3 px-3">Factura</th>
            ...
            <tbody>
              {turnoInvoices.map((inv) => (
                <tr key={inv.id} className="border-b border-white/5">
                  <td className="py-3 px-3 text-white text-sm font-medium">{inv.invoiceNumber}</td>
                  <td className="py-3 px-3 text-white text-sm">{inv.clientName}</td>
                  <td className="py-3 px-3 text-right text-sm font-medium" style={{ color: inv.status === 'cancelled' ? '#ef4444' : '#fff' }}>
                    US$ {inv.total.toFixed(2)}
                  </td>
                  <td className="py-3 px-3 text-center">
                    <span className={`text-xs px-2 py-0.5 rounded-full ${inv.status === 'cancelled' ? 'bg-red-500/20 text-red-400' : 'bg-green-500/20 text-green-400'}`}>
                      {inv.status === 'cancelled' ? 'Cancelada' : 'Activa'}
                    </span>
                  </td>
                </tr>
              ))}
              {turnoInvoices.length === 0 && (
                <tr><td colSpan={4} className="py-8 text-center text-white/50">No hay facturas en este turno</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function CierreDiaPanel({ invoices }) {
  ...
  const todayStr = new Date().toLocaleDateString('es-DO');
  const todayInvoices = invoices.filter(i => i.date === todayStr);
  const activeInvoices = todayInvoices.filter(i => i.status !== 'cancelled');
  const cancelledInvoices = todayInvoices.filter(i => i.status === 'cancelled');
  const approvedReturns = returns.filter(r => {
    const inv = todayInvoices.find(i => i.invoiceNumber === r.invoice);
    return inv && r.status === 'aprobada';
  });

  const turnoNames = ['Turno 9:00', 'Turno 12:00', 'Turno 3:00'];
  const turnoBreakdown = turnoNames.map(t => {
    const tInvoices = activeInvoices.filter(i => i.turno === t);
    const tCancelled = cancelledInvoices.filter(i => i.turno === t);
    return { turno: t, ventas: tInvoices.length, total: tInvoices.reduce((s, i) => s + i.total, 0), canceladas: tCancelled.length };
  });

  const totalVentas = activeInvoices.reduce((s, i) => s + i.total, 0);
  const totalSubtotal = activeInvoices.reduce((s, i) => s + i.subtotal, 0);
  const totalTax = activeInvoices.reduce((s, i) => s + i.tax, 0);
  const totalDevoluciones = approvedReturns.reduce((s, r) => s + r.amount, 0);
  const totalNeto = totalVentas - totalDevoluciones;

  const handlePrint = () => {
    const w = window.open('', '', 'width=302,height=900');
    const turnoRows = turnoBreakdown.map(t =>
      '<tr><td style="padding:3px 0;">' + t.turno + '</td><td style="text-align:center;">' + t.ventas + '</td><td style="text-align:right;">US$ ' + t.total.toFixed(2) + '</td></tr>'
    ).join('');
    const invoiceRows = activeInvoices.map(i =>
      '<tr><td style="padding:2px 0;font-size:10px">' + i.invoiceNumber + '</td><td style="font-size:10px">' + (i.clientName || '').substring(0, 15) + '</td><td style="text-align:right;font-size:10px">US$ ' + i.total.toFixed(2) + '</td></tr>'
    ).join('');

    w.document.write('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Cierre del Dia</title>'
      + '<style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:monospace;width:80mm;padding:8px;font-size:12px}'
      + 'h2{text-align:center;font-size:14px;margin-bottom:4px}table{width:100%;border-collapse:collapse}'
      + '.sep{border-top:1px dashed #000;margin:8px 0}.total{font-size:18px;font-weight:bold;text-align:center;margin:8px 0}'
      + '@page{size:80mm auto;margin:0}@media print{body{width:80mm}}</style></head><body>'
      + '<h2>MACAO PHOTOGRAPHY</h2><p style="text-align:center;font-size:11px">CIERRE DEL DIA</p>'
      + '<p style="text-align:center;font-size:11px">' + todayStr + '</p>'
      + '<div class="sep"></div>'
      + '<p style="font-weight:bold;margin-bottom:4px">RESUMEN POR TURNO</p>'
      + '<table><tr style="font-size:10px;border-bottom:1px solid #000"><th style="text-align:left">Turno</th><th style="text-align:center">Cant</th><th style="text-align:right">Total</th></tr>' + turnoRows + '</table>'
      + '<div class="sep"></div>'
      + '<p style="font-weight:bold;margin-bottom:4px">TODAS LAS FACTURAS (' + activeInvoices.length + ')</p>'
      + '<table>' + invoiceRows + '</table>'
      + (cancelledInvoices.length > 0 ? '<div class="sep"></div><p style="font-weight:bold;">CANCELADAS: ' + cancelledInvoices.length + '</p>' : '')
      + '<div class="sep"></div>'
      + '<table>'
      + '<tr><td>Subtotal:</td><td style="text-align:right">US$ ' + totalSubtotal.toFixed(2) + '</td></tr>'
      + '<tr><td>ITBIS (18%):</td><td style="text-align:right">US$ ' + totalTax.toFixed(2) + '</td></tr>'
      + '<tr><td>Total Ventas:</td><td style="text-align:right">US$ ' + totalVentas.toFixed(2) + '</td></tr>'
      + '<tr><td>Devoluciones:</td><td style="text-align:right">-US$ ' + totalDevoluciones.toFixed(2) + '</td></tr>'
      + '</table>'
      + '<div class="sep"></div>'
      + '<div class="total">NETO: US$ ' + totalNeto.toFixed(2) + '</div>'
      + '<div class="sep"></div>'
      + '<p style="text-align:center;font-size:10px">Total facturas: ' + todayInvoices.length + ' | Canceladas: ' + cancelledInvoices.length + '</p>'
      + '<p style="text-align:center;font-size:10px">Impreso: ' + new Date().toLocaleString('es-DO') + '</p>'
      + '</body></html>');
    w.document.close();
    setTimeout(() => w.print(), 400);
  ...
  return (
    <div className="flex-1 flex flex-col">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-black/25 backdrop-blur-xl rounded-3xl p-5 border border-white/20">
          <p className="text-white/60 text-xs mb-1">Facturas</p>
          <p className="text-white text-3xl font-bold">{activeInvoices.length}</p>
        </div>
        <div className="bg-black/25 backdrop-blur-xl rounded-3xl p-5 border border-white/20">
          <p className="text-white/60 text-xs mb-1">Total Ventas</p>
          <p className="text-green-400 text-2xl font-bold">US$ {totalVentas.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
        </div>
        <div className="bg-black/25 backdrop-blur-xl rounded-3xl p-5 border border-white/20">
          <p className="text-white/60 text-xs mb-1">Canceladas</p>
          <p className="text-red-400 text-3xl font-bold">{cancelledInvoices.length}</p>
          <p className="text-red-400 text-sm">-US$ {totalDevoluciones.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
        </div>
        <div className="bg-black/25 backdrop-blur-xl rounded-3xl p-5 border border-[#DC2626]/40">
          <p className="text-white/60 text-xs mb-1">Total Neto</p>
          <p className="text-[#DC2626] text-2xl font-bold">US$ {totalNeto.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
        </div>
      </div>

      {/* Turno Breakdown */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        {turnoBreakdown.map((t) => (
          <div key={t.turno} className="bg-black/25 backdrop-blur-xl rounded-3xl p-5 border border-white/20">
            <p className="text-white font-semibold mb-2">{t.turno}</p>
            <div className="flex justify-between text-sm">
              <span className="text-white/60">{t.ventas} ventas</span>
              <span className="text-white font-medium">US$ {t.total.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
            </div>
            {t.canceladas > 0 && <p className="text-red-400 text-xs mt-1">{t.canceladas} cancelada{t.canceladas > 1 ? 's' : ''}</p>}
          </div>
        ))}
      </div>
      ...
      {/* Detail Table */}
      <div className="flex-1 bg-black/25 backdrop-blur-xl rounded-3xl p-5 border border-white/20 overflow-auto">
        <h3 className="text-white font-semibold mb-4">Todas las Facturas del Dia</h3>
        <table className="w-full">
          <thead>
            <tr className="text-white/70 text-xs border-b border-white/10">
              <th className="text-left py-3 px-3">Factura</th>
              ...
              <th className="text-right py-3 px-3">ITBIS</th>
              <th className="text-right py-3 px-3">Total</th>
              <th className="text-center py-3 px-3">Estado</th>
            </tr>
          </thead>
          <tbody>
            {todayInvoices.map((inv) => (
              <tr key={inv.id} className="border-b border-white/5">
                <td className="py-3 px-3 text-white text-sm font-medium">{inv.invoiceNumber}</td>
                <td className="py-3 px-3 text-white text-sm">{inv.clientName}</td>
                <td className="py-3 px-3 text-white/70 text-sm">{inv.turno}</td>
                <td className="py-3 px-3 text-right text-white/70 text-sm">US$ {inv.subtotal.toFixed(2)}</td>
                <td className="py-3 px-3 text-right text-white/70 text-sm">US$ {inv.tax.toFixed(2)}</td>
                <td className="py-3 px-3 text-right text-sm font-medium" style={{ color: inv.status === 'cancelled' ? '#ef4444' : '#fff' }}>
                  US$ {inv.total.toFixed(2)}
                </td>
                <td className="py-3 px-3 text-center">
            ...
            {todayInvoices.length === 0 && (
              <tr><td colSpan={7} className="py-8 text-center text-white/50">No hay facturas hoy</td></tr>
            )}
          </tbody>
        </table>
      </div>
